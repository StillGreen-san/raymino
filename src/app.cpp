...
namespace raymino {
	...
	App::App() : playerName{"Mino"}, keyBindsPresets{{"Default", {}}},
	...
	bool App::addHighScore(int64_t score) {
		const bool isHighScore = highScores.add(playerName.data(), score,
		                                        settingsPresets.get(activeSettingsPreset).value);
	...
	SaveFile App::decompressFile(const void *compressedData, uint32_t size) {
		...
		const int decompressedSize = ::sinflate(&decompressedData[HeaderSize], static_cast<int>(inputHeader->userProp3),
		                                        inputHeader + 1,
		                                        static_cast<int>(size - HeaderSize));
	...
	SaveFile App::serialize() const {
		...
		const auto settingsPresetsSize = static_cast<uint32_t>(
			settingsPresets.size() * sizeof(Presets<Settings>::Item));
		...
		save.appendChunk(ChunkType::KeyBindsPresets, static_cast<uint16_t>(activeKeyBindsPreset),
		                 keyBindsPresets.get().begin() + static_cast<ptrdiff_t>(keyBindsPresets.fixed()),
		                 keyBindsPresets.get().end());
		save.appendChunk(ChunkType::SettingsPresets, static_cast<uint16_t>(activeSettingsPreset),
		                 settingsPresets.get().begin() + static_cast<ptrdiff_t>(settingsPresets.fixed()),
		                 settingsPresets.get().end());
